...
import { VENDORS, orderMailtos } from './data/vendors'
    ...
    <div className="app">
      ...
      {tab === 'tasks' &&
        (() => {
          ...
          // one-click order emails (same drafts as the Materials tab's ✉️
          // buttons — usually one, two for a Block & Lintels row). Computed
          // here (only when the Tasks tab is open) so TasksView stays a dumb
          // renderer. projById avoids an O(n²) find per pending order.
          ...
          const pending = collectPendingOrders(projects, getProjectState).map((po) => {
            ...
            const drafts = proj
              ? orderMailtos(vendors, po.category, proj, ps, state.templates, state.modelOrderLists?.[modelKey(proj.model)])
              : []
            return {
              ...po,
              drafts: drafts.map((d) => ({ href: d.href, vendorName: d.vendor.name, portion: d.portion })),
            }
          })
          return (
            <TasksView
      ...
      {tab === 'projects' &&
        (reporting ? (
          ...
          <Detail
            ...
            applyVerifiedWaterUtility={applyVerifiedWaterUtility}
            applyScoutedWaterUtility={applyScoutedWaterUtility}
