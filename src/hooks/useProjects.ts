...
import { mergeBlockLintels, mergeTrussesFraming } from '../lib/orders'
...
export function migrate(parsed: Partial<WorkbenchState>): WorkbenchState {
  ...
  for (const [id, ps] of Object.entries(parsed.projects ?? {})) {
    ...
    // Legacy split category pairs → their combined rows (Aug 2026):
    // 'Block'+'Lintels' → "Block & Lintels", 'Trusses'+'Framing package' →
    // "Trusses & Framing". Idempotent, so they simply run on every load — no
    // one-time flag; see lib/orders.ts mergeCategoryPair for the exact rules.
    if (norm.orders?.length) norm.orders = mergeTrussesFraming(mergeBlockLintels(norm.orders))
