...
## nocForm.ts — the Notice of Commencement, filled & print-ready (Aug 2026)

Fills Marion County's NOC (PMT 5 Rev 9-20-22, public/templates/NOC_BLANK.pdf)
for the Permit tab's "Notice of Commencement — fill & print" card. The NOC is
signed by William as Owner, notarized, RECORDED, and posted on the job site
BEFORE the first inspection (FS 713.13). Rules (Adam, Aug 25 2026):

- Fill ONLY informational fields; signature/date/title/notary stay blank.
- Line 1 = the PA's FULL legal (data/paProperty.ts PA_RAW_LEGAL — plat book
  included) + street address; falls back to the compact legal, then the
  look-it-up placeholder. Never silently blank.
- Owner 3a = the county PA's DEEDED owner (PA_OWNER). Iron Shield-deeded
  lots → the Belleview office address. Anything else (investor, Mr Ocala
  Buys Houses, …) shows verbatim + a "verify on Sunbiz / PA may lag a recent
  deed" nudge. Always editable.
- Lender (6) is ALWAYS N/A — financed houses get their NOC from the lender.
- Contractor block is fixed: "Iron Shield Construction, LLC" /
  "William Stiles PO BOX 5651, Ocala FL, 34478" / 352-342-2206.
- data/paProperty.ts is GENERATED by scripts/fetch-legals.mjs --write (same
  pass as legal.ts) — re-run after adding projects.
- Field coordinates come from the blank's own label layout, visually
  verified against a recorded example (Book 8701/Page 1536). Re-measure,
  never nudge by eye. pdf-lib stays a lazy import.
